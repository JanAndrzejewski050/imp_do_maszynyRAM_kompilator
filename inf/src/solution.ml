...
let rec compile_bexp (e : bexp) (stack : var list) : cmd list =
  match e with
  | Bool b -> if b then [CONST 1] else [CONST 0]
  | Cmp (op, a_e1, a_e2) -> compile_aexp a_e1 stack @ [PUSH] @ compile_aexp a_e2 ("none" :: stack) @ [CMP op]
  | And (b_e1, b_e2) -> compile_bexp b_e1 stack @ [BRANCH (compile_bexp b_e2 stack, [CONST 0])]
  | Or (b_e1, b_e2) -> compile_bexp b_e1 stack @ [BRANCH ([CONST 1], compile_bexp b_e2 stack)]
  | Not b_e -> compile_bexp b_e stack @ [BRANCH ()]
  | _ -> []
